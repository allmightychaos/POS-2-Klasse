...
int main(int, char**) {
    ...
    // string palindromes[] = {"Erika feuert nur ugntreue Fakire", "Never odd or even"};
    // int count = 2;
    ...
    for(int i = 0; i < count; i++) {
        try
        {
            ...
            {
                cout << "\"" << palindromes[i] << "\"" << " ist \e[1mein\e[0m Palindrom" << endl;
            }
            else
            {
                cout << "\"" << palindromes[i] << "\"" << " ist \e[1mkein\e[0m Palindrom" << endl;
            }
        }
        catch(const std::exception& e)
        {
